...
class ProgramGenerator(private val program: ProgramNode) {
        ...
        return TypeSpec.objectBuilder(programName)
            ...
            .apply {
                ...
                program.instructions.forEach { instruction ->
                    ...
                    if (instruction.name == "transferSol") {
                        generateDeprecatedWrapper(instruction, mapOf(
    ...
    private fun generateDeprecatedWrapper(instruction: InstructionNode, oldParamNames: Map<String, String>): FunSpec? {
        if (oldParamNames.isEmpty()) return null
        
        val functionName = when (instruction.name) {
        ...
        return FunSpec.builder(functionName)
            ...
            .addAnnotation(
                AnnotationSpec.builder(Deprecated::class)
                    ...
                    .addMember("replaceWith = ReplaceWith(%S)", instruction.name.toCamelCase() + "(" + 
                        buildString {
                            ...
                            instruction.accounts.forEach { account ->
                                ...
                                if (!shouldSkip) {
                                    val oldName = oldParamNames[account.name.toCamelCase()] ?: account.name.toCamelCase()
                                    params.add("$oldName = $oldName")
                                }
                            }
                            nonDiscriminatorArgs.forEach { arg ->
                                val oldName = oldParamNames[arg.name.toCamelCase()] ?: arg.name.toCamelCase()
                                params.add("${arg.name.toCamelCase()} = $oldName")
                            }
            ...
            .apply {
                instruction.accounts.forEach { account ->
                    ...
                    if (!shouldSkip) {
                        val paramName = oldParamNames[account.name.toCamelCase()] ?: account.name.toCamelCase()
                        addParameter(
                            paramName,
                            ClassName("net.avianlabs.solana.tweetnacl.ed25519", "PublicKey")
                        )
                ...
                nonDiscriminatorArgs.forEach { arg ->
                    val oldParamName = oldParamNames[arg.name.toCamelCase()] ?: arg.name.toCamelCase()
                    addParameter(
                        oldParamName,
                        LONG
                    )
            ...
            .addStatement("return %L(%L)", 
                ...
                buildString {
                    ...
                    nonDiscriminatorArgs.forEach { arg ->
                        val oldName = oldParamNames[arg.name.toCamelCase()] ?: arg.name.toCamelCase()
                        // Convert Long to ULong for amount/lamports
                        val converted = when (arg.type.format) {
                            "u64" -> "$oldName.toULong()"
                            "u8" -> "$oldName.toUByte()"
